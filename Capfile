...
set :nhosts, 1
set :group_name, 'aSVZ_RESTChIPseq'
...
task :upload_data, :roles => group_name do
    puts "rsync -e 'ssh -i /home/spkaabi/ec2/spkaabi.pem' -vzP data/* ubuntu@ec2-46-51-128-78.eu-west-1.compute.amazonaws.com:#{mount_point}" 
   end
...
task :run_macs, :roles => group_name do
  ...
  treatment = "#{mount_point}/CMN064_export_sorted_nodups.bam"
  control = "#{mount_point}/CMN063_export_sorted_nodups.bam"
  genome = 'mm'
  bws = [300]
  pvalues = [0.00001]
